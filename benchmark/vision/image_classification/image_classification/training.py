...
from actnn_aq import config, QScheme, QModule, get_memory_usage, compute_tensor_bytes, exp_recorder
...
def get_optimizer(parameters, fp16, lr, momentum, weight_decay,
    ...
    #optimizer = AGC([v for n, v in parameters], optimizer, clipping=0.005)
...
def get_train_step(model_and_loss, optimizer, fp16, use_amp = False, batch_size_multiplier = 1):
    def _step(input, target, optimizer_step = True):
        ...
        if config.qmode == 'actnn-aq' and config.aq_bit < 1.0:
            torch_utils.clip_grad_norm_(model_and_loss.model.parameters(),
                                        max_norm=1.0,
                                        norm_type=2
                                    )
