...
from gact_aq.conf import config
import gact_aq.cpp_extension.quantization as ext_quantization
import gact_aq.cpp_extension.minimax as ext_minimax
from torch.utils.checkpoint import checkpoint
import gact_aq.cpp_extension.calc_precision as ext_calc_precision

def no_scheme_quantize_pack(input, q_bit, seed):
    group_size = config.group_size
    if config.aq:
        group_size = int(config.group_size * config.aq_bit)
    
    N = (input.numel() + group_size - 1) //  group_size
    num_ele = N * group_size
    pad_num = num_ele - input.numel()
    if pad_num > 0:
        input = torch.cat([input.view(1, -1), torch.zeros([1, pad_num], 
...
def op_quantize(input, q_bit, seed):
    ...
    if config.aq:    
        if q_bit == 1: # apply average and quantization when AQ 0.x-bit is allocated
            ...
            return [q_input, q_bit, q_scale, q_min, q_input_shape, None, remainder, pad_num, delta]
        else: # apply pure quantization when N-bit is allocated
...
def op_dequantize(input, input_shape):
    q_input, q_bit, q_scale, q_min, q_input_shape, q_input_shape2, remainder, pad_num, delta = input
