...
from actnn_aq.utils import get_memory_usage, compute_tensor_bytes, empty_cache, swap_to_cpu
import actnn_aq.cpp_extension.quantization as ext_quantization
import actnn_aq.cpp_extension.minimax as ext_minimax
import actnn_aq.cpp_extension.backward_func as ext_backward_func
 
from actnn_aq.averaging import average_and_pack, repeat_and_unpack
...
def quantize_and_pack(data, bits, mn, mx):
    ...
    return output, scale, data.shape


def dequantize_and_unpack(data, shape, bits, scale, mn, group_size=config.group_size):
    if config.simulate:
        data = data / scale + mn
    else:
        if config.swap:
...
def no_scheme_compute_quantization_bits(input):
    ...
    if num_features % config.group_size != 0:
        ...
        padding = new_num_features - num_features
        input_flatten = torch.cat([input_flatten,
                                   torch.zeros([N, padding], dtype=input.dtype, device=input.device)], 1)
...
def quantize_activation(input, scheme, qmode):
    ...
    if not config.compress_activation:
        ...
        return [input, None, None, None, None, None, None, None], None, None, None, None, None, None, None
    
    if scheme:
        input_groups, q_bits, q_min, mx, padding = scheme.compute_quantization_bits(input)
    else:
        input_groups, q_bits, q_min, mx, padding = no_scheme_compute_quantization_bits(input)

    
    if qmode == 'actnn-plain':
        q_input, q_scale, before_quantize_shape = quantize_and_pack(input_groups, q_bits, q_min, mx)
        
    elif qmode == 'actnn-aq':
        if not(isinstance(q_bits, int)): # if we allocate bit to input activations differently by batch index
            ind1 = (q_bits!=1).nonzero(as_tuple=True)[0] # batch index of activations which is pure N-bit
            ind2 = (q_bits==1).nonzero(as_tuple=True)[0] # batch index of activations which is pure AQ 0.x-bit
        
        else: # if we allocate bit to input activations identically by batch index
            if q_bits != 1:
                ind1 = torch.empty(1, device=input.device)
                ind2 = torch.empty(0, device=input.device)
            else:
                ind1 = torch.empty(0, device=input.device)
                ind2 = torch.empty(1, device=input.device)
            
        if len(ind2) == 0: # if we allocate only pure N-bit to input activations
            q_input, q_scale, before_quantize_shape = quantize_and_pack(input_groups, q_bits, q_min, mx)
             
        elif len(ind1) == 0: # if we allocate only AQ 0.x-bit to input activations
            input_groups, q_min, mx, remainder, padding2, before_average_shape = average_and_pack(input_groups.detach(), padding) # averaging
            q_input, q_scale, before_quantize_shape = quantize_and_pack(input_groups, 2, q_min, mx) # quantization
            
        else:
            '''
            if we allocate both pure N-bit and AQ 0.x-bit to input activations.
            we have to divide, average, and concat by batch index.
            '''
            # divide tensor by bits (ind1, ind2)
            N = input_groups.size(0)
            input_groups1, input_groups2 = input_groups.detach()[ind1,:,:], input_groups.detach()[ind2,:,:]  #[N1,H,config.group_size], [N2,H,config.group_size]
            
            # averaging of input_groups2, information of averaging 
            input_groups2, q_min2, mx2, remainder, padding2, before_average_shape = average_and_pack(input_groups2, padding) # results:[N',H,config.group_size]
            q_bits2 = 2*torch.ones(input_groups2.size(0), dtype=torch.int32, device=input_groups2.device) # [N']
            N1 = input_groups1.size(0) # number of samples
            N2 = N-N1
            q_bits1 = torch.index_select(q_bits, dim=0, index=ind1) # [N1]
            q_min1 = torch.index_select(q_min, dim=0, index=ind1) # [N1]
            mx1 = torch.index_select(mx, dim=0, index=ind1) # [N1]
            
            # remove cache for aq, it alleviate memory fragmentation
            if config.remove_cache:                
                empty_cache(0.5)
            
            # concatenating input_groups1 and averaging(input_groups2)
            input_groups, q_bits, q_min, mx = torch.cat([input_groups1, input_groups2],0), torch.cat([q_bits1,q_bits2]), torch.cat([q_min1,q_min2]), torch.cat([mx1,mx2])  # [N1+N',H,config.group_size], [N1+N']..
            
            q_input, q_scale, before_quantize_shape = quantize_and_pack(input_groups, q_bits, q_min, mx)
        
    # TODO convert q_bits to int8
    if input.dtype == torch.float and q_scale != None:
        return [q_input, q_bits, q_scale.to(torch.bfloat16), q_min.to(torch.bfloat16), ind1, ind2], before_quantize_shape, before_average_shape, padding, padding2, remainder, N1, N2
    else:
        return [q_input, q_bits, q_scale, q_min, ind1, ind2], before_quantize_shape, before_average_shape, padding, padding2, remainder, N1, N2
        

def dequantize_activation(quantized, qmode, q_input_shape, before_quantize_shape=None, before_average_shape=None, padding=0, padding2=0, remainder=0, N1=0, N2=0):
    if not config.compress_activation:
        ret = quantized[0]
        if config.swap:
            ret = ret.cuda(non_blocking=True)
        return ret
    q_input, q_bits, q_scale, q_min, ind1, ind2 = quantized
    if q_scale != None:
    ...
    if qmode == 'actnn-plain':
        ...
        elif len(ind1) == 0: # if we allocate bit to input activations identically by batch index
            input = dequantize_and_unpack(q_input, before_quantize_shape, 2, q_scale, q_min) #[N, H, config.group_size]
            input = repeat_and_unpack(input.detach(), before_average_shape, remainder, padding2)
            
        else:
            '''
            if we allocate both pure N-bit and AQ 0.x-bit to input activations.
            we have to divide, repeat, and merge by batch index.
            '''
            H = before_quantize_shape[1]
            
            # dequantization
            input_groups = dequantize_and_unpack(q_input, before_quantize_shape, q_bits, q_scale, q_min)
            del q_input
            
            # dividing averaging (input_groups2)
            input_groups2 = input_groups.detach()[N1:,:,:] #[N', H, config.group_size]
            
            # repeating of averaging and reshaping
            input_groups2 = repeat_and_unpack(input_groups2, before_average_shape, remainder, padding2)
            if padding != 0:
                input_groups2 = torch.cat([input_groups2, torch.empty([N2, padding], dtype=input_groups2.dtype, device=input_groups2.device)], 1)
            input_groups2 = input_groups2.view(N2,-1,config.group_size) #[N2, H, config.group_size]
            
            # dividing input_gorups1
            input_groups1 = input_groups[:N1,:,:] #[N1, H, config.group_size]
            del input_groups
...
class convnd(Function):
    ...
    def run_forward(n, forward_op, ctx, input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1, scheme=None):
        ...
        quantized, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2 = quantize_activation(input, scheme, config.qmode)

        ctx.scheme = scheme
        ctx.saved = quantized, weight, bias
        ctx.other_args = (input.shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2, stride, padding, dilation, groups)
    ...
    def run_backward(n, ctx, grad_output, bias_reduce_dims, aug):
        ...
        q_input_shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2, stride, padding, dilation, groups = ctx.other_args
        padding = aug(padding)
        stride = aug(stride)
        dilation = aug(dilation)

        quantized, weight, bias = ctx.saved
        input = dequantize_activation(quantized, config.qmode, q_input_shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2)
...
class conv_transposend(Function):
    ...
    def run_forward(n, forward_op, ctx, input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1, scheme=None):
        ...
        ctx.scheme = scheme
        ctx.saved = quantized, weight, bias
        ctx.other_args = (input.shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, stride, padding, output_padding, dilation, groups)
    ...
    def run_backward(n, ctx, grad_output, bias_reduce_dims, aug):
        ...
        q_input_shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2, stride, padding, output_padding, dilation, groups = ctx.other_args
        padding = aug(padding)
...
class linear(Function):
    ...
    def forward(ctx, input, weight, bias=None, scheme=None):
        ...
        ctx.scheme = scheme
        ctx.saved = quantized, weight, bias
        ctx.other_args = input.shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2
    ...
    def backward(ctx, grad_output):
        ...
        quantized, weight, bias = ctx.saved
        q_input_shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2 = ctx.other_args
        ...
        else: # 3D input for Transformer in Linear layer 
...
class sync_batch_norm(Function):
    ...
    def forward(self, input, weight, bias, running_mean, running_var, eps, momentum, process_group, world_size, scheme):
        ...
        self.scheme = scheme
        self.other_args = input.shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2
    ...
    def backward(self, grad_output):
        ...
        q_input_shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2 = self.other_args
        saved_input = dequantize_activation(quantized, config.qmode, q_input_shape, before_quantize_shape, before_average_shape, padding1, padding2, remainder, N1, N2)
