...
torch::Tensor calc_precision(torch::Tensor b, torch::Tensor C, torch::Tensor w, double target, double aq_bit) {
    /*
    Sensitivity is computed in the same way as in the previous ActNN,
    but aq_bit is used instead of 1-bit when calculating the total bit budget. 
    As a result, the 1-valued elements in the output tensor b represent AQ 0.x-bit, not pure 1-bit.
    */
    ...
    while (b_sum > target) {        // Pick up the smallest increment (largest decrement)
        ...
        if (b_data[i]==1) {b_sum -= (2 - aq_bit)*w_data[i];} // aq_bit is used instead of 1-bit when calculating the total bit budget.
        else {b_sum -= w_data[i];}
