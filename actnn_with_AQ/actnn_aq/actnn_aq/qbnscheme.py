...
from actnn_aq.qscheme import QScheme
...
class QBNScheme(QScheme):
    ...
    def compute_quantization_bits(self, input):
        ...
        if not config.pergroup:    # No per group quantization
            mn = torch.ones_like(mn, device=input.device) * mn.min()
            mx = torch.ones_like(mx, device=input.device) * mx.max()
        ...
        if config.qmode == 'actnn-aq':
            b = ext_calc_precision.calc_precision(b, C, w, int(self.bits * N), config.aq_bit)
        elif config.qmode == 'actnn-plain':
            b = ext_calc_precision.calc_precision(b, C, w, int(self.bits * N), 1)
        b = b.to(input.device)
    
        return input_groups.view(N, -1, config.group_size).detach().contiguous(), b, mn.view(N, -1, 1), mx.view(N, -1, 1), delta
