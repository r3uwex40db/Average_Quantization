...
import actnn_aq.cpp_extension.calc_precision as ext_calc_precision
import numpy as np
...
class QScheme(object):
    ...
    def compute_quantization_bits(self, input):
        ...
        # greedy
        grad_sum = self.get_scale().to(input.device)
        ...
        if config.qmode == 'actnn-aq':
            b = ext_calc_precision.calc_precision(b, C, w, int(self.bits * N), config.aq_bit)         # N
        elif config.qmode == 'actnn-plain':
            b = ext_calc_precision.calc_precision(b, C, w, int(self.bits * N), 1)                     # N
        b = b.to(input.device)
        
        self.C = C
        self.dim = input.numel() // N
        self.b = b
        return input_groups.view(N, -1, config.group_size).contiguous(), b, mn.view(N, -1, 1), mx.view(N, -1, 1), delta
    ...
    def allocate_perlayer():
        ...
        for g in range(num_groups):
            ...
            if config.activation_compression_bits[g] == config.initial_bits:
                ...
                w = torch.tensor([layer.dim for layer in layers], dtype=torch.int32)
                total_bits = w.sum() * config.activation_compression_bits[g]
                b = torch.ones(L, dtype=torch.int32) * 8
                if config.qmode == 'actnn-aq':
                    b = ext_calc_precision.calc_precision(b, C, w, total_bits, config.aq_bit)
                elif config.qmode == 'actnn-plain':
                    b = ext_calc_precision.calc_precision(b, C, w, total_bits, 1)
                for i in range(L):
                ...
                # Ws = [torch.ones(N, dtype=torch.int64) for layer in layers]
                w = torch.cat(Ws, 0)
                ...
                for i in range(L):
                    bs = b[i*N : (i+1)*N]
                    if config.qmode == 'actnn-aq':
                        bs = (bs==1)*config.aq_bit + (bs!=1)*bs # replace 1-bit to aq_bit
    ...
    def if_allocate_perlayer(self):
        ...
        if config.compress_activation and config.training:
            if self == first_layer:
                ...
                actnn_aq.QBNScheme.allocate_perlayer()
