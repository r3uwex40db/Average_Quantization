...
import actnn_aq
from actnn_aq import config, QScheme, QModule
from actnn_aq import get_memory_usage, compute_tensor_bytes, exp_recorder
MB = 1024**2
GB = 1024**3
...
parser.add_argument('--aq-bit', type=float, default=0, help='average quantization bit')
parser.add_argument('--bit', type=float, default=0, help='bit')

best_acc1 = 0

def set_optimization_level(args, bit, aq_bit):
    if args.alg == 'exact':
        pass
    elif args.alg.startswith('actnn-'):
        actnn_aq.set_optimization_level(args.alg[6:], bit=bit, aq_bit=aq_bit)
    elif args.alg == 'swap':
        actnn_aq.set_optimization_level('swap', bit=bit, aq_bit=aq_bit)
    else:
...
def main():
    ...
    set_optimization_level(args, bit=args.bit, aq_bit=args.aq_bit)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader = actnn_aq.dataloader.DataLoader(
    ...
    if args.get_macs:
        ...
        from thop.vision.basic_hooks import count_convNd, count_normalization, count_linear
        from actnn_aq.layers import QConv2d, QBatchNorm2d, QLinear
        ...
        macs, params = profile(model, inputs=(input, ),
            custom_ops={QConv2d: count_convNd, QBatchNorm2d: count_normalization, QLinear: count_linear})
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (indices, (images, target)) in enumerate(train_loader):
        ...
        if config.debug_memory_model:
            ...
            res = "Batch size: %d\tTotal Mem: %.2f GB\tAct Mem: %.2f GB" % (
                    len(output), before_backward / GB, act_mem / GB)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            del loss
            ...
            res = "Batch size: %d\tTotal Mem: %.2f GB\tAct Mem: %.2f GB" % (
                    len(output), total_mem / GB, act_mem / GB)
            print(res)
...
if __name__ == '__main__':
    ...
    #allocate cache memory first
    #x = torch.randn(int(2.7*2**31), device='cuda:6')
    #del x 
