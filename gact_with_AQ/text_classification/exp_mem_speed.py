...
def network_to_command(network):
    cmd = "python run_glue.py --model_name_or_path ARCH --task_name sst2 --max_length 128 " + \
        ...
        "--num_train_epochs 1 --seed 42 --pad_to_max_length --device 3"
...
def binary_search_max_hidden_size(alg, low, high, network, batch_size):
    ...
    while low <= high:
        ...
        success = (run_benchmark(network, alg, hidden_size=mid, batch_size=batch_size, intermediate_size=4*mid,
                                 debug_speed=True) == 0)
        if success:
            ret = mid
...
def get_ips(network, alg, batch_size, hidden_size=None, layer_num=None, intermediate_size=None):
    run_benchmark(network, alg, batch_size, layer_num=layer_num,
                  hidden_size=hidden_size, debug_speed=True)
...
if __name__ == "__main__":
    ...
    if args.mode == 'linear_scan':
        ...
        algs = ['L2.2_', 'L2.2_GN', 'L2.2_GN_AQ0.5', 'L2.2_GN_AQ0.5_1.5b', 'L2.2_GN_AQ0.5_1.0b']
    ...
    if args.mode == 'linear_scan':
        ...
        for network in networks:
            for alg in algs:
                ...
                macs, params = get_macs(network, alg, max_batch_size)
                ...
                with open(out_file, "a") as fout:
                    val_dict = {
                        ...
                        "macs": macs,
                        "params": params,
                        "TFLOPS": round(macs * ips / max_batch_size / 1e12, 2),
                        "tstamp": time.time()
                    }
                    fout.write(json.dumps(val_dict) + "\n")
                print(f"save results to {out_file}")
        ...
        for alg in algs:
            ...
            macs, params = get_macs(network, alg, batch_size, hidden_size=max_hidden_size)
